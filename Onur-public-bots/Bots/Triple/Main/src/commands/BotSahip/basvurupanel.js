...
const { onurxSonsuz, onurxStar, onurx1, onurx2, onurx3, onurx4, onurx5, onurx6, onurx7, onurx8, onurx9, onurxTik, onurxRed } = require("../../../../src/configs/emojis.json")
...
client.on('interactionCreate', async interaction => {
    ...
    if(interaction.customId === "basvuru") {
        if (!interaction.user.displayName.includes(conf.tag)) {
            interaction.reply({content: `${onurxRed} öncelikle sunucumuzun tagını alman gerekiyor. Tagımız: \`${conf.tag}\``, ephemeral:true});
            return
    };
        ...
        let s5 = interaction.fields.getTextInputValue('soru5') || "\`Boş Bırakılmış\`";
            await interaction.reply({content: `${onurxTik} Yetkili başvurun başarıyla alındı`, ephemeral: true});
        ...
        if(client.channels.cache.find(x => x.name == "başvuru-log")) {
            ...
            await channelxd2.send(`> ${onurxTik} **${member} Yetkililik Başvurun Kabul Edilmiştir Edilmiştir.**\n> \n> **Kabul Eden Yetkili: ${interaction.user}**`);
            await interaction.update({content:`> ${onurxTik} **Yetkili Başvurusu Başarıyla Kabul Edildi!**\n> **Kabul Eden Yetkili:** ${interaction.user}`,components: []});
            ...
            await channelxd.send(`> ${onurxRed} **${member} Yetkililik Başvurun Red Edilmiştir.**\n> \n> **Red Eden Yetkili: ${interaction.user}**`);
            await interaction.update({content:`> ${onurxRed} **Yetkili Başvurusu Red Edildi!**\n> **Red Eden Yetkili:** ${interaction.user}`,components: []});
