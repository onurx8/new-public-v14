...
const { onurxSonsuz, onurxStar, onurx1, onurx2, onurx3, onurx4, onurx5, onurx6, onurx7, onurx8, onurx9 } = require("../../../../src/configs/emojis.json")
...
${onurxSonsuz} \`(${total} gerçek, ${bonus} bonus, ${leave} ayrılmış, ${fake} fake)\`
      
${onurxSonsuz} \`Günlük: ${daily}, Haftalık: ${weekly}, Taglı: ${tagged}\`
`, ephemeral: true });
}
...
{
await interaction.reply({ content: `
${onurxSonsuz} Sesli kanallardaki üye sayısı : \`${(interaction.guild.members.cache.filter((x) => x.voice.channel).size)}\`
${onurxSonsuz} Sunucudaki toplam üye sayısı : \`${(interaction.guild.memberCount)}\`
${onurxSonsuz} Sunucunun oluşturulma tarihi: \`${moment(interaction.guild.createdAt).locale("tr").format("LLL")}\`
${onurxSonsuz} Sunucu destek numarası : \`${(interaction.guild.id)}\`
`, ephemeral: true });
}
...
${onurxStar} **Mesaj İstatistiği**
${onurxSonsuz} Toplam: \`${messageData ? messageData.topStat : 0}\`

${onurxSonsuz} Haftalık Mesaj: \`${Number(messageWeekly).toLocaleString()} mesaj\`
${onurxSonsuz} Günlük Mesaj: \`${Number(messageDaily).toLocaleString()} mesaj\`
`, ephemeral: true });
}
...
${onurxStar} **Sesli Sohbet İstatistiği**
${onurxSonsuz} Toplam: \`${moment.duration(voiceData ? voiceData.topStat : 0).format("H [saat], m [dakika] s [saniye]")}\`
