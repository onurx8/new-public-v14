...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    await ozicik.react(onurxTik);

    const filter = (reaction, user) => {
        return reaction.emoji.name === 'onurxTik' && user.id === message.author.id;
    };
    const collector = ozicik.createReactionCollector({ filter, time: 10000 });

    collector.on("collect", async () => {
        await ozicik.edit({ content: `${onurxTik} ${members.length} adet kullanıcı başarıyla yasaklandı.`})
