...
module.exports = {
  conf: {
    ...
    name: "vmute",
    help: "vmute <onurx/ID> <Süre> <Sebep>",
  ...
  run: async (client, message, args, embed) => {
    ...
    if (duration) {
      ...
      if(msg) msg.delete();
      await message.channel.send({ content:`${onurxTik} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **sesli kanallarda** susturuldu! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
       message.react(onurxTik)
    ...
    await interaction.deferUpdate();
    if(msg) msg.delete();
    interaction.followUp({ content: `${onurxTik} Sesli Susturma işlemi başarıyla iptal edildi.`, ephemeral: true });
    }
    })
    }
      },
