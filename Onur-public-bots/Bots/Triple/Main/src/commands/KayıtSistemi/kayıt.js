...
${onurxRed} üyesinin toplamda **${data ? `${data.names.length}` : "0"}** isim kayıtı bulundu
${data ? data.names.splice(-3).map((x, i) => `\` ${i + 1} \` \` ${x.name} \` ${x.sebep ? `(${x.sebep})` : ""} ${x.rol ? `(${x.rol})` : ""}`).join("\n") : "Daha önce kayıt olmamış."}

Üyesinin önceki isimlerine \`.isimler <@onurx/ID>\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.
`)
...
if(msg) msg.delete();
message.reply({ content:`İşlem Başarıyla İptal Edildi ${onurxTik}`, embeds: [], components: []});
