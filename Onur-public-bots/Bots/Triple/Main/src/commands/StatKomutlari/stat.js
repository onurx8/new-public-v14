...
const { onurxStar, onurxSonsuz, onurxOk } = require("../../../../src/configs/emojis.json");
    ...
    // There should be no further addField() call here. If you need to add more fields, use addFields() method again.
      ...
      .addFields(
...

        {
          ...
          name: `${onurxStar} **Sesli Sohbet İstatistiği**`,
          value: `
          ${onurxSonsuz} **Genel Toplam Ses :** \`${moment.duration(voiceData ? voiceData.topStat : 0).format("H [saat], m [dakika]")}\`
          ${onurxSonsuz} **Genel Toplam Mesaj :** \`${messageData ? messageData.topStat : 0} mesaj\`
      
          ${onurxSonsuz} **Haftalık Ses :** \`${voiceWeekly}\`
          ${onurxSonsuz} **Haftalık Chat :** \`${Number(messageWeekly).toLocaleString()} mesaj\`
      
          ${onurxSonsuz} **Günlük Ses :** \`${voiceDaily}\`
          ${onurxSonsuz} **Günlük Chat :** \`${Number(messageDaily).toLocaleString()} mesaj\`
      
          ${onurxSonsuz} **Davetleri :** **${total}** (**${regular}** gerçek, **${bonus}** bonus, **${leave}** ayrılmış, **${fake}** fake)
          ${onurxSonsuz} **Daha geniş çaplı bilgilere erişmek için lütfen aşağıdaki münüyü kullanınız!**`,
          inline: false
        })
      ...
      collector.on("collect", async (interaction) => {
        ...
        if (interaction.values[0] === "stat1") {
          ...
          if (liste.length > 0) {
            embeds.addFields({
              name: `${onurxStar} **Sesli Kanal İstatistiği**`, value: `
${liste.slice(page == 1 ? 0 : page * 10 - 10, page * 10).join('\n')}
`, inline: false
            })
          }
          embeds.addFields({
            name: `${onurxStar} **Sesli Kategori İstatistiği**`, value: `
${onurxSonsuz} Toplam: \`${moment.duration(voiceData ? voiceData.topStat : 0).format("H [saat], m [dakika]")}\`
${onurxSonsuz} Public Odalar: \` ${await category(conf.publicParents)} \`
${onurxSonsuz} Secret Odalar: \` ${await category(conf.privateParents)} \`
${onurxSonsuz} Alone Odalar: \` ${await category(conf.aloneParents)} \`
${onurxSonsuz} Yönetim Yetkili Odaları: \` ${await category(conf.funParents)} \`
${onurxSonsuz} Kayıt Odaları: \` ${await category(conf.registerParents)} \`
...
${onurxStar} \`[ ${regular} gerçek, ${bonus} ekstra, ${leave} ayrılmış, ${fake} sahte ]\`
      
${onurxOk} Günlük: \` ${daily} \`, Haftalık: \` ${weekly} \`, Taglı: \` ${tagged} \`
