...
const { onurxKalp, onurxStaff, onurxTik ,onurxStar , onurxd2, onurxb2, onurxd1, onurxb3, onurxd3, onurxRed, onurxSonsuz } = require("../../../../src/configs/emojis.json");
...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if(!conf.staffs.some(rol => message.member.roles.cache.has(rol))) return message.react(onurxRed);
...
${onurxStaff} ${member.toString()}, **Günlük / Haftalık / Toplam Ses İstatistikleri;**

${onurxSonsuz} \` Toplam Ses İstatistiği :  \` **${voiceDaily}**
${onurxSonsuz} \` Haftalık Ses İstatistiği :  \` **${voiceWeekly}**
${onurxSonsuz} \` Günlük Ses İstatistiği :  \` **${voiceDaily}**

${onurxStaff} ${member.toString()}, **Günlük / Haftalık / Toplam Mesaj İstatistikleri;**

${onurxSonsuz} \` Toplam Mesaj İstatistiği : \` **${messageData ? messageData.topStat : 0}**
${onurxSonsuz} \` Haftalık Mesaj İstatistiği : \` **${Number(messageWeekly).toLocaleString()}**
${onurxSonsuz} \` Günlük Mesaj İstatistiği : \` **${Number(messageDaily).toLocaleString()}**

${onurxStaff} ${member.toString()}, **Genel Yetili/Yetki Yükseltim İstatistiklerin Aşağıda Listeleniyor;**

${onurxSonsuz} \` Yetki İlerleme Durumu :  \` ${progressBar(coinData ? coinData.coin : 0, maxValue.coin, 9)}
${onurxSonsuz} \` Yetki Yükselim Puanı :  \` [**${maxValue.coin}**] 
${onurxSonsuz} \` Bulunan Yetkin :  \` ${message.member.roles.highest}
${onurxSonsuz} \` Yükselinecek Yetkin :  \` ${Array.isArray(maxValue.role) ? maxValue.role.length > 1 ? maxValue.role.slice(0, -1).map(x => `<@&${x}>`).join(', ') + ' ve ' + maxValue.role.map(x => `<@&${x}>`).slice(-1) : maxValue.role.map(x => `<@&${x}>`).join("") : `<@&${maxValue.role}>`}
${onurxSonsuz} \` İlerleme Yüzdesi :  \` ilerleme durumun **${atlamaBilgisi}** yapıyorsun.

${onurxStaff} ${member.toString()}, **Görev İstatistiklerin Aşağıda Listeleniyor;**

${onurxSonsuz} \` Görev Alma Tarihi :  \` Bulunamadı. 
${onurxSonsuz} \` Görev Bitiş Tarih :  \` ${mtask.length ? mtask.filter((x) => x.active).map((x) => `**${moment.duration(x.finishDate - Date.now()).format("H [saat], m [dakika] s [saniye]")}**`).join("\n\n") : "Veri bulunamadı."}      
${onurxSonsuz} \` Görev Puanı :  \` **20**
${onurxSonsuz} \` Görev Türü :  \` ${mtask.length ? mtask.filter((x) => x.active).map((x) => `${x.message}`).join("\n\n") : "Veri bulunamadı."}
${onurxSonsuz} \` Görev İlerleme :  \` ${mtask.length === 0 ? "Görev Seçmelisin !" : mtask.filter((x) => x.active).map((x) => `\n${x.completedCount >= x.count ? `${onurxTik}` + " **Tamamlandı!**" : `${progressBar(x.completedCount, x.count, 8)} \`${x.type === "ses" || x.type === "yayin" || x.type == "camera" ? `${moment.duration(x.completedCount).format("H [saat], m [dk], s [sn]")} / ${moment.duration(x.count).format("H [saat], m [dk], s [sn]")}` : `${x.completedCount} / ${x.count}`}\``}`).join("\n\n")}

${onurxStaff} ${member.toString()}, **Top Ses/Mesaj/Kamera/Yayın Sıralaması İstatistiklerin Aşağıda Listeleniyor;**

${onurxSonsuz} \` Top Ses Sıralaması :  \` **${sıralama2}**
${onurxSonsuz} \` Top Mesaj Sıralaması :  \` **${sıralama}**
${onurxSonsuz} \` Top Yayın Sıralaması :  \` **${sıralama3}**
${onurxSonsuz} \` Top Kamera Sıralaması :  \` **${sıralama4}**
`)
...
.addFields({ name:`${onurxStar} **Puan Durumu:**`, value:`
...
`, inline: false },
{ name:`${onurxStar} **Yetki Durumu:**`, value:`
${coinStatus}
`, inline: false })
...
`) 
.addFields({ name:`${onurxStar} **Ceza Kullanımı**`, value: `\`\`\`fix
...
`, inline: false },
{ name:`${onurxStar} **Ceza Puan Detayları:**`, value: `
${onurxKalp} (\` Ban işlemi \`) yerseniz, \`-100\` puan kaybedersiniz.
${onurxKalp} (\` Underworld \`) işlemi yerseniz, \`-75\` puan kaybedersiniz.
${onurxKalp} (\` Karantina/Jail \`) işlemi yerseniz, \`-50\` puan kaybedersiniz.
${onurxKalp} (\` Ses/Yazı \`) Mute işlemi yerseniz, \`-20\` puan kaybedersiniz.
`, inline: false },
{ name:`${onurxStar} **Puan Durumu:**`, value: `
Puanınız: \`${coinData ? Math.floor(coinData.coin) : 0}\`, Gereken Puan: \`${maxValue.coin}\`
${progressBar(coinData ? coinData.coin : 0, maxValue.coin, 9)} \`${coinData ? coinData.coin : 0} / ${maxValue.coin}\`
`, inline: false },
{ name:`${onurxStar} **Yetki Durumu:**`, value:`
...
.addFields({ name:`${onurxStar} **Sesli Sohbet İstatistiği**`, value:`
${onurxKalp} Toplam: \`${moment.duration(voiceData ? voiceData.topStat : 0).format("H [saat], m [dakika]")}\`
${onurxKalp} Public Odalar: \`${await category(conf.publicParents)}\`
${onurxKalp} Secret Odalar: \`${await category(conf.privateParents)}\`
${onurxKalp} Alone Odalar: \`${await category(conf.aloneParents)}\`
${onurxKalp} Yönetim Yetkili Odaları: \`${await category(conf.funParents)}\`
${onurxKalp} Kayıt Odaları: \`${await category(conf.registerParents)}\`
`, inline: false },
{ name:`${onurxStar} **Mesaj İstatistiği**`, value:`
${onurxKalp} Toplam: \`${messageData ? messageData.topStat : 0}\`
${onurxKalp} Haftalık Mesaj: \`${Number(messageWeekly).toLocaleString()} mesaj\`
${onurxKalp} Günlük Mesaj: \`${Number(messageDaily).toLocaleString()} mesaj\`
`, inline: false });
...
function progressBar(value, maxValue, size) {
  ...
  const progressText = onurxd2.repeat(progress);
  const emptyProgressText = onurxb2.repeat(emptyProgress);
  
  return emptyProgress > 0 ? onurxd1+progressText+emptyProgressText+onurxb3 : onurxd1+progressText+emptyProgressText+onurxd3;
};
