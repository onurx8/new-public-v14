...
const { onurxRed, onurxb2, onurxb3, onurxd1, onurxd2, onurxd3, onurxTik } = require("../../../../src/configs/emojis.json")
module.exports = {
  conf: {
    aliases: ["gorevler", "görevlerim", "yapmamgerekenler","gorevlerim"],
    name: "görevler",
    help: "görevler @onurx/ID",
    category: "yetkili"
  },
  ...
  run: async (client, message, args, embed) => {
    ...
    if (!conf.staffs.some(rol => member.roles.cache.has(rol))) return message.react(onurxRed)
    ...
    function progressBar(value, maxValue, size) {
      const fill = `${onurxd2}`;
      const fillStart = `${onurxd1}`;
      const fillEnd = `${onurxd3}`;
      const empty = `${onurxb2}`;
      const emptyEnd = `${onurxb3}`;
...
${mtask.filter((x) => x.active).map((x) => `\`#${x.id}\` ${x.message} \n${x.completedCount >= x.count ? `${onurxTik}` + " **Tamamlandı!**" : `${progressBar(x.completedCount, x.count, 8)} \`${x.type === "ses" || x.type === "yayin" || x.type == "camera" ? `${moment.duration(x.completedCount).format("H [saat], m [dk], s [sn]")} / ${moment.duration(x.count).format("H [saat], m [dk], s [sn]")}` : `${x.completedCount} / ${x.count}`}\` \nKalan Süre: \`${moment.duration(x.finishDate - Date.now()).format("H [saat], m [dakika] s [saniye]")}\` \nÖdül: **${x.prizeCount} puan**`}`).join("\n\n")}        
