...
const { onurxOk, onurxTik, onurxRed ,onurxStar } = require("../../../../src/configs/emojis.json");
...
`})
    ...
    collector.on("collect", async (button) => {
      ...
      .setDescription(`${onurxTik} ${member.toString()} üyesinin isim geçmişi ${message.author} tarafından \`${moment(Date.now()).format("LLL")}\` tarihinde temizlendi!`)
...
msg.edit({
  ...
  if(button.customId === "cezapuan_sıfırla") {
    ...
    .setDescription(`${onurxTik}  ${member.toString()} üyesinin ceza puanı ${message.author} tarafından \`${moment(Date.now()).format("LLL")}\` tarihinde temizlendi!`) 
...
msg.edit({
 ...
 if(button.customId === "sicil_sıfırla") {   
    ...
    .setDescription(`${onurxTik}  ${member.toString()} üyesinin sicili ${message.author} tarafından \`${moment(Date.now()).format("LLL")}\` tarihinde temizlendi!`) 
...
msg.edit({
  ...
  components : []
    ...
    if(button.customId === "stat_sıfırla") {   
      ...
      .setDescription(`${onurxTik}  ${member.toString()} üyesinin stat verileri ${message.author} tarafından \`${moment(Date.now()).format("LLL")}\` tarihinde temizlendi!`) 
 ...
 if(button.customId === "iptal_button") {   
    ...
    .setDescription(`${onurxTik} Sıfırlama işlemi iptal edildi`) 
