...
const { onurxStar, onurxRed, onurxTik } = require("../../../../src/configs/emojis.json");
...
embed.addFields({ name: `${onurxStar} Son Toplantı Bilgisi`, value: `
> Toplantıya **\`${MeetingData.Joining.length}\`** yetkili katılmış ${onurxTik}
> Toplantıya **\`${MeetingData.Leaving.length}\`** yetkili katılmamış ${onurxRed}
...
**${guild.name}** sunucusunda şuan da toplantı durumu: **${meetingStatus ? "Aktif" : "Devre-dışı"}** ${meetingStatus ? onurxTik : onurxRed}`)
...
${onurxStar} **Bitirilen Toplantı Bilgisi**
> Bu toplantıda katılması beklenen **(\`${KatilimData.Leaving.length + KatilimData.Joining.length} yetkili\`)**
> Bu toplantıda katılım sağlayan: **(\`${KatilimData.Joining.length} yetkili\`)** ${onurxTik}
> Bu toplantıda katılım sağlamayan: **(\`${KatilimData.Leaving.length} yetkili\`)** ${onurxRed}
