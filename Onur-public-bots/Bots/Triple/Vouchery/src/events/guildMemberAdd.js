...
const { onurxStar, onurxTik, onurxRed, onurxOk, onurxGiris } = require("../../../src/configs/emojis.json")
  ...
  if(res.taglar.some(x => member.displayName.includes(x))) { 
    ...
    if (allah.Main.dmMessages) member.send({ content:`${member.guild.name} adlı sunucumuza olan erişiminiz engellendi! Sunucumuzda yasaklı olan bir simgeyi (${x}) isminizde taşımanızdan dolayıdır. Sunucuya erişim sağlamak için simgeyi (${x}) isminizden çıkartmanız gerekmektedir.\n\nSimgeyi (${x}) isminizden kaldırmanıza rağmen üstünüzde halen Yasaklı Tag rolü varsa sunucudan gir çık yapabilirsiniz veya sağ tarafta bulunan yetkililer ile iletişim kurabilirsiniz. **-Yönetim**\n\n__Sunucu Tagımız__\n**${conf.tag}**`}).catch(() => {});
		...
		const welcomeEmbed2 = new EmbedBuilder()
            .setColor("#fcfafb")
            .setDescription(`
${onurxStar} ${member} Seni Aramızda Görmek Çok Güzel. Sunucumuz Seninle Birlikte ${üyesayısı} Kişi olduk!

${onurxTik} Hesabın <t:${Math.floor(member.user.createdTimestamp / 1000)}> (<t:${Math.floor(member.user.createdTimestamp / 1000)}:R>) Oluşturulmuş.

${onurxTik} Sunucumuza <t:${Math.floor(Date.now() / 1000)}:R> Zamanında Katıldınız.

${onurxOk} Tagımıza Ulaşmak İçin Herhangi Bir Kanala \`.tag\` Yazabilirsiniz.
            `)
...
channel.send({ content:`${onurxGiris} ${member}, sunucuya katıldı! Davet Eden: **Sunucu Özel URL** Sunucumuz **${member.guild.memberCount}** Uye sayisine ulaşti ${emoji.onurxCekilis}`})
return }
...
kayitchannel.send({ content:`${onurxTik} ${member} isimli üye sunucuya katıldı fakat hesabı (<t:${Math.floor(member.user.createdTimestamp / 1000)}:R>) açıldığı için şüpheli olarak işaretlendi.`});
...
channel.send({ content:`${onurxGiris} ${member}, ${usedInvite.inviter.tag} davetiyle katıldı! Uyenin Davet Sayisi (**${total}**) Sunucumuz **${member.guild.memberCount}** Uye sayisine ulaşti`})
}
